    ...
    if (ok == true) {
      setState(() {
        ...
        if (wasReset && index < rows.length) {
          // A reset row was deleted — use the safe boundary-change recompute
          // which fixes ODO values BEFORE rebuilding derived fields.
          recomputeAfterResetBoundaryChange(rows, index);
        } else {
          // No reset boundary changed — normal recompute is fine.
          recomputeRollchartDerived(rows);
        }
        carrySurface = rows.isEmpty ? SurfaceType.DT : rows.last.surface;
      });
